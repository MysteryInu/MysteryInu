...
contract mysteryinu is Context, IERC20, Ownable {
    ...
    address public developmentWallet = 0x1a63B89f8a2655c4E211D4325f36094c0B417965;
    uint256 private _previousdevelopmentFee = _developmentFee;
	
	uint256 public _marketingFee = 2;
    address public marketingWallet = 0xDF8054b5F61346E2E84135773fC3D6cBCF2AE8ae;
    uint256 private _previousmarketingFee = _marketingFee;
	
	uint256 public _rewardsFee = 4;
    address public rewardsWallet = 0xc67cda0CCf679aF8C8e4d63784030Bd0f2275C0f;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _burnFee==0 && _rewardsFee ==0 && _developmentFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
		...
		(tTransferAmount, rTransferAmount) = takeRewards(sender, tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
	...
	function takeRewards(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256){
        if(_rewardsFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tRewards = tAmount.div(100).mul(_rewardsFee);
        uint256 rRewards = tRewards.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rRewards);
        tTransferAmount = tTransferAmount.sub(tRewards);
        _rOwned[rewardsWallet] = _rOwned[rewardsWallet].add(rRewards);
        emit Transfer(sender, rewardsWallet, tRewards);
        return(tTransferAmount, rTransferAmount);
    }
